...
namespace riku
{
  template<typename T> var<T>::var() : variant() {}
  template<typename T> var<T>::var(var<T> const& rhs) : variant((variant&)rhs) {};
  template<typename T> var<T>::~var() { variant::~variant(); }

  template<typename T>
  var<T>::var(variant_type const& rhs) : variant() {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
  }
  template<typename T>
  var<T>::var(variant_type& rhs) : variant() {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
  }
  template<typename T>
  var<T>::var(variant const& rhs) : variant() {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
  }
  template<typename T>
  var<T>::var(variant& rhs) : variant() {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
  }

  template<typename T>
  var<T>& var<T>::operator=(variant_type const& rhs) {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
    return *this;
  }
  template<typename T>
  var<T>& var<T>::operator=(variant const& rhs) {
    if (rhs.type() != NULL && rhs.type()->has_parent(riku::get<T>())) variant::operator=(rhs);
    return *this;
  }

  template<typename T> var<T>::operator T*() {
    return reinterpret_cast<T*>(data());
  }
  template<typename T> var<T>::operator T const*() const {
    return reinterpret_cast<T const*>(data());
  }
  template<typename T> T& var<T>::operator*() { return *(operator T*()); }
  template<typename T> T* var<T>::operator->() { return operator T*(); }
